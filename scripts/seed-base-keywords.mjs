...
const CATEGORY_RULES = [
  ...
  //
  // 최초 기획(src/types.ts)의 9개 공종을 다 덮도록 채웠다. 전에는 누수·전기·환기
  // 규칙이 아예 없어서 그 공종 키워드가 전부 fallback으로 떨어졌다.
  //
  // 순서를 정한 근거 — 이름이 두 공종에 걸치는 경우가 많다:
  //   '렌지후드'는 후드지만 주방 공사다 → 주방을 환기보다 앞에
  //   '욕실환풍기'는 환풍기지만 욕실 공사다 → 욕실을 환기보다 앞에
  //   '누수천장도배'는 도배 공사다. '누수'가 먼저 걸리면 방수로 새므로 도배를 앞에 둔다.
  //     반대로 '누수탐지'에는 도배 단어가 없어 그대로 방수로 간다.
  //   '방충망'은 문이 아니라 환기·기타 공종이다 → 환기를 문보다 앞에 두고
  //     '현관롤방충망'이 '현관' 때문에 문으로 새는 것을 막는다.
  ['kitchen-sink', ['싱크대', '씽크대', '수전', '주방', '후드', '음식물', '분쇄기', '디스포저']],
  ['bathroom-toilet', ['변기', '샤워', '욕실', '화장실', '세면대', '세면기']],
  ['wallpaper-flooring', ['도배', '벽지', '마루', '바닥', '장판', '계단']],
  ['leak-waterproof', ['누수', '방수', '물샘', '코킹', '결로']],
  ['electrical-lighting', ['전기', '조명', '차단기', '누전', '콘센트', '스위치', '등기구', '인덕션', '배선']],
  ['piping-heating-boiler', ['수도', '배관', '설비', '보일러', '하수구', '막힘', '동파', '역류']],
  ['screen-vent-etc', ['방충망', '환풍기', '환기']],
  ['door-window-sash', ['문', '도어', '현관', '방화문', '문틀', '문지방', '문턱', '중문', '자동문', '유리문', '폴딩도어', '샷시', '필름']],
