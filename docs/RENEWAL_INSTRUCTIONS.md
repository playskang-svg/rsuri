...
> **2026-09-11 기준 변경.** 사장님이 준 참고 스킨(가상 이사서비스 판매 스킨 "차곡길 이사연구소" 화면 17장)으로 레이아웃·디자인을 다시 맞췄다.
> 크림 바탕 · 차콜 · 로열블루 · 머스터드 · 주황 CTA, 굵은 고딕(Noto Sans KR 900) + 모노 라벨(IBM Plex Mono),
> 색 블록 히어로(홈 머스터드 · 허브 파랑 · 지역·사례 차콜 · 분야 베이지 · 사이트맵 머스터드), 섹션 머리(라벨+두 줄 제목+오른쪽 설명),
> 번호 칸 · 윗선 색 카드 · 굵은 윗줄 FAQ, 모든 페이지 푸터 위 어두운 상담 띠. 공통 부품은 `web/app/_components/PageHero.tsx`·`CtaBand.tsx`.
> 아래 네이비+골드 내용은 그 이전 결정이다. URL·DB·조립 로직(1·2·6번)은 이번 전환에서도 그대로다.

참고 사이트(이전): `koreajipsurimaster.com` (네이비+골드 계열)
