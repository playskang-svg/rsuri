...
export function buildFixtures() {
  ...
  for (const kw of keywords) {
    for (const region of sigungu) {
      pages.push({
        ...
        source_case_id: null,
        search_intent: `${region.display_name}에서 ${kw.display_name}는 어떻게 진행하는가?`,
        target_phrase: `${region.display_name} ${kw.display_name}`,
        intended_content_type: 'CT1',
        content_type_fell_back: false,
        hold_reason: null,
        required_modules: ['M01', 'M03', 'M04', 'M09'],
        selected_modules: ['M21', 'M24'],
        module_order: ['M01', 'M03', 'M04', 'M09', 'M21', 'M24'],
        evidence_ids: [`keyword-content:${kw.slug}`],
        image_set: [],
  ...
  if (firstKeyword) {
    pages.push({
      ...
      source_case_id: null,
      search_intent: '문이 아래쪽만 끌릴 때 현장에서는 무엇을 보고 판단했는가?',
      target_phrase: '문 처짐 수리',
      intended_content_type: 'CT6',
      content_type_fell_back: false,
      hold_reason: null,
      required_modules: ['M02', 'M03', 'M06', 'M08', 'M18', 'M20', 'M27'],
      selected_modules: ['M21', 'M24'],
      module_order: ['M27', 'M02', 'M03', 'M06', 'M08', 'M18', 'M20', 'M21', 'M24'],
      evidence_ids: ['fixture-case-door-sag'],
      image_set: [],
      meta_title: '문 처짐 — 경첩 교체와 문틀 보정',
      meta_description: '닫을 때 아래쪽만 끌리던 방문. 경첩 나사 구멍이 헐거워진 게 원인이었습니다.',
      decision: 'CREATE',
      region_profile_id: sigungu[1]?.id ?? sigungu[0].id,
      merged_into_page_id: null,
      diy_vs_pro: null,
      area_served: null,
      seo_keywords: [],
