// 참고 사진 (Unsplash 핫링크).
//
// 2026-09-10: 문 계열을 Unsplash API로 다시 골랐다. 전에는 door 4장을 100개 넘는
// 문 키워드가 돌려써서 어느 페이지를 열어도 같은 사진이 나왔다. alt 설명을 실제로
// 확인하고 담았고, 경첩·손잡이는 door-hardware로 따로 뺐다 — 처짐·헐거움처럼
// 부속이 주제인 키워드에 문짝 전신 사진은 "무엇을 고치는지"를 보여주지 못한다.
//
...
const POOL: Record<string, string[]> = {
  ...
  // 문 계열이 키워드의 절반 이상이라 가장 여러 장을 둔다.
  // 2026-09-10 Unsplash API로 다시 골랐다 — 전에는 4장을 100개 넘는 문 키워드가
  // 돌려써서 같은 사진이 계속 나왔다. alt 설명을 실제로 확인하고 담았다.
  door: [
    P('1777589555626-0c11eb48418d'), // 한국 전통 목문 — 국내 사이트에 가장 자연스럽다
    P('1603673298820-40d77252226d'), // 흰 벽 옆 원목문
    P('1764592119348-0aaeab50e2d0'), // 스포트라이트 아래 모던 원목문
    P('1786465220058-41cbeb61e775'), // 사선 판재 양개문
    P('1769971343851-6cb873927415'), // 어두운 실내의 문과 손잡이
    P('1542354642-233af003db87'),
    P('1542020383883-6c6b2a343807'),
  ],

  // ── 경첩·손잡이 클로즈업 ──
  // 처짐·헐거움·교체처럼 부속이 주제인 키워드는 문 전체 사진보다 이쪽이 맞다.
  // 문짝 전신 사진은 "무엇을 고치는지"를 보여주지 못한다.
  'door-hardware': [
    P('1779204749779-5296e620968a'), // 원목문에 물린 은색 경첩
    P('1765750376628-6c986f112ca7'), // 오래된 문의 장식 경첩
    P('1588704821128-889309f68363'), // 흰 문의 레버 손잡이
    P('1574588822710-475a38fe3371'), // 원목문 레버 손잡이
    P('1734207184552-338108c78336'), // 손잡이 클로즈업
  ],
  stairs: [
  ...
  floor: [
    ...
    P('1611072337226-1140ab367200'),
    P('1573869908170-64b53a60d8da'), // 원목 마루 결
  ],
  ...
  'kitchen-sink': [
    ...
    P('1745794565783-913db01a89b9'), // 싱크대와 선반이 함께 잡힌 주방
  ],
  'leak-waterproof': [
...
const TOPIC_RULES: [string, string[]][] = [
  ...
  // 부속이 주제인 키워드는 문 전체 사진이 아니라 부속 클로즈업이 맞다.
  // door보다 앞에 둬야 '문고리교체'가 door로 새지 않는다.
  ['door-hardware', ['경첩', '손잡이', '문고리', '도어록', '레버']],
  ['floor', ['마루', '바닥', '장판']],
  ['wallpaper', ['도배', '벽지']],
  ['window', ['샷시', '샤시', '창호', '창문']],
  ['door', ['문', '도어', '현관', '필름', '시트지']],
